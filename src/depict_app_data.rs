...
impl DepictAppData {
    pub fn new(fetched_data_set: &FetchedDataSet) -> Self {
        let mut display_data_set = DisplayDataSet::new(&fetched_data_set.list_all_depiction_category());
        ...
        for depiction in fetched_data_set.list_all_depiction_category().clone() {
            ...
            display_data_set.to_display.insert(depiction.clone(), ArcSwap::from_pointee(built));
        };
                    ...
                    match fetched_data_set.entries[entry_pos].perform_update_if_needed(current_time) {
                        ...
                        Ok(true) => {
                            for depiction in &fetched_data_set.entries[entry_pos].depict {
                                let map_entries =
                                    fetched_data_set.build_data_for_depiction_category(depiction.clone());
                                let display_entry = display_data_set.to_display.get(depiction).unwrap(); // All possible value should be set in the constructor (althought a bad implementation could lead to a panic here)
                                display_entry.swap(Arc::new(map_entries));
                                info!("Update successfull for {:?}", fetched_data_set.entries[entry_pos].fetcher.title());
                            }
                        }
                        Err(err) => {
                            warn!(
